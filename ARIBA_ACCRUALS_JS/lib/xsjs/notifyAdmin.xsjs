...
async function sendMail(subject, body, emails) {
	...
	try {
		...
		res = "Message ID = " + Res_1.messageId + ", Final Reply = " + Res_1.finalReply;
		return res;
	} catch (e) {
...
async function fullName(name) {
	try {
		...
		const result = connection.executeQuery(requestInfo, name);
		return result;
	} catch (e) {
		return e.message.toString();
	}
...
async function sendMailAdmin(dash, email) {
	let res = "";
	try {
		...
		const result = await connection.executeQuery(poRequestorQuery, "NOTIFY_ADMIN", "X", userLanguage);
		...
		for (var y = 0; y <= jobSchedule.length; y++) {
			...
			if (currDate === jobDate) {
				...
				for (var k = 0; k < adminResult.length; k++) {
					// var guidquery= connection.executeQuery("SELECT NEWUID() \"ID\" FROM \"ARIBA_ACCRUAL.ARIBA_ACCRUALS_DB.synonyms::DUMMY\"");
					...
					var podetails = adminResult[k].RESPONSE;
					var email = adminResult[k].EMAILID;
					...
					await connection.commit();
					var res = await sendMailAdmin(poAdmin, adminResult[k].EMAILID);
