...
async function sendMailreq(dash, email) {
	let res = "";
	try {
		...
		const result = await connection.executeQuery(poRequestorQuery, "NOTIFY_POOWNER", "X", userLanguage);
...
async function jobStart() {
	try {
		...
		const jobSchedules = result.$resultSets[0];

		console.log("Job Schedules: ", jobSchedules, "PO Owners: ", poownerResult);
		// to get current date
		...
		for (let jobSchedule of jobSchedules) {
			var jobDate = getNewDateString(jobSchedule.JOB_DATE);
			...
			for (var j = 0; j < poownerResult.length; j++) {
				...
				var podetails = poownerResult[j].RESPONSE;
				...
				var res = await sendMailreq(poRequestor, poownerResult[j].EMAILID);
