...
async function sendMail(attachment, senderAddr, userLangResult) {
	...
	var tempQueryResult = await connection.executeQuery(tempQuery, "POCLOSURE_EMAIL", userLangResult);
...
try {
	...
	if (res.length !== 0) {
		...
		const senderAddrQuery =
			'SELECT "CONFIG_VALUE" FROM "ARIBA_ACCRUAL.ARIBA_ACCRUALS_DB.synonyms::ZT_CONFIG_VALUES" WHERE "CONFIG_NAME" = ?';
		const result = await connection.executeQuery(senderAddrQuery, "DAILY_POCLOSURE_EMAIL_TO");
		const senderAddr = result[0].CONFIG_VALUE.split(/[ ,]+/);

		var i = 0;

		for (i in senderAddr) {
			// to get user browser language
			const sentAddr = senderAddr[i];
			const userLang =
				'SELECT "LANGUAGE" FROM "ARIBA_ACCRUAL.ARIBA_ACCRUALS_DB.synonyms::ZT_USER_LANGUAGE" WHERE "EMAIL_ID" = ?';
			const userLangExc = await connection.executeQuery(userLang, sentAddr);
			let userLangResult = "";
